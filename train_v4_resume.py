"""
V4 Resume Training Script - Continue from V4 best.pth.

Purpose: Resume V4 training from epoch 35 to 50 (15 additional epochs).

Key design:
  1. Load only model weights from V4 best.pth (no optimizer/scheduler)
  2. New AdamW optimizer with LR=1e-6 (V4 ended at ~1e-6)
  3. CosineAnnealingLR: T_max=15, eta_min=1e-7
  4. Independent checkpoint/log directories
  5. Identical training logic to V4 (GradScaler, CPU augmentation, etc.)

Architecture: UNCHANGED from V4
  - ViT (DINO v1 vits8, frozen)
  - Mamba (3 layers x 4 blocks, d_model=512)
  - SpatialSoftmax (K=16)
  - DiffusionActionHead (down_dims=64,128,256)
"""
...
BATCH_SIZE = 48
EPOCHS = 50  # Total epochs (V4 completed 35, resume to 50 = 15 more)
LR = 1e-6  # Resume LR (V4 ended at ~1e-6)
WARMUP_EPOCHS = 0  # No warmup for resume
MIN_EPOCHS = 3
PATIENCE = 8
GRAD_CLIP = 1.0
GRAD_CLIP_MAX = 2.0
SEED = 42
CKPT_DIR = "checkpoints_diffusion_v4_resume"
LOG_DIR = "logs_diffusion_v4_resume"
V4_CKPT = "checkpoints_diffusion/best.pth"
...
def load_model_weights(path, vision, core, head, spatial_softmax, keypoint_proj):
    ...
    return ckpt.get("epoch", -1), ckpt.get("best_loss", float("inf"))
...
def train():
    ...
    print("=" * 60)
    print("V4 Resume Training (epoch 35 -> 50)")
    print("   Device: %s" % DEVICE)
    print("   Resume from: %s" % V4_CKPT)
    ...
    # Load V4 model weights only
    if not os.path.exists(V4_CKPT):
        print("V4 checkpoint not found: %s" % V4_CKPT)
        return

    print("Loading V4 model weights...")
    v4_epoch, v4_best_loss = load_model_weights(
        V4_CKPT, vision, core, head, spatial_softmax, keypoint_proj
    )
    print("  V4 epoch: %d, best_loss: %.6f" % (v4_epoch, v4_best_loss))
    ...
    # CosineAnnealingLR only (no warmup for resume)
    scheduler = CosineAnnealingLR(optimizer, T_max=EPOCHS - v4_epoch - 1, eta_min=1e-7)

    loader = get_diffusion_loader(batch_size=BATCH_SIZE, num_workers=4)

    logger = MetricsLogger(log_dir=LOG_DIR)

    start_epoch = v4_epoch + 1
    best_loss = v4_best_loss
    patience_counter = 0

    print("  Starting from epoch %d, best_loss=%.6f" % (start_epoch, best_loss))
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if avg_loss < best_loss and epoch >= MIN_EPOCHS:
            best_loss = avg_loss
            patience_counter = 0
            save_checkpoint(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--v4-ckpt", type=str, default=None, help="V4 checkpoint path")
    parser.add_argument("--epochs", type=int, default=None, help="Total epochs")
    parser.add_argument("--lr", type=float, default=None, help="Learning rate")
    args = parser.parse_args()

    if args.v4_ckpt:
        V4_CKPT = args.v4_ckpt
    if args.epochs:
        EPOCHS = args.epochs
    if args.lr:
        LR = args.lr
